...
for i=1:handles.data.N
    ...
    if(isfield(handles.data, "SynchroCluster"))
...
[filepath, name, ext] = fileparts(handles.data.filename);
separator = filesep;
file_components = strsplit(filepath, separator);
output_path = fullfile(file_components{:}, 'single_cell.csv');
writematrix(dat, output_path);

handles.dat = dat;
columnname = {'Baseline fluorescence','Total events','<ISI> (s)','Amplitude','CV','Rise time (s)','Fall time (s)'};
columneditable = [false false false false false false false];

if(isfield(handles.data, "SynchroCluster") & isfield(handles.data, "FC"))
